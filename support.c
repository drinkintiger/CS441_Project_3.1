...
int allocate_matrix( mtype_t **matrix, int buffer_size ) {
    size_t alloc_size;

    // Page align the allocation
    alloc_size = sizeof(mtype_t) * buffer_size * buffer_size;
    ...
    for(i = 0; i < buffer_size; ++i ) {
        for(j = 0; j < buffer_size; ++j ) {
            if( sizeof(mtype_t) == sizeof(double) ) {
                matrix[ GET_INDEX(i, j, buffer_size)] = (random() % 100) / 10.0;
            }
            else {
                matrix[ GET_INDEX(i, j, buffer_size)] = random() % 10;
            }
        }
    }
}
